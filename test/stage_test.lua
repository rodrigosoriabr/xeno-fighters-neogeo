-- with a STAGE_TEST build: coin + start, then a snapshot every 100 frames (9 stages, ~700 frames each)
...
emu.register_frame_done(function()
  ...
  if f > 1100 and f % 100 == 0 then manager.machine.video:snapshot() end
  if f >= 8000 then manager.machine:exit() end
end)
